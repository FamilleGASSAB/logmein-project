...
def health():
    ...
    try:
        ...
        return (
            jsonify({"status": "error", "database": "disconnected", "error": str(e)}),
            500,
        )
...
def add_log():
    ...
    try:
        ...
        return jsonify({"success": True, "log": response_log}), 201
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def get_stats():
    ...
    try:
        ...
        if total == 0:
            return jsonify(
                {"total_logs": 0, "levels": {}, "services": {}, "last_log": None}
            )
...
def clear_logs():
    ...
    try:
        ...
        return jsonify({"success": True, "message": f"{deleted_count} logs cleared"})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
